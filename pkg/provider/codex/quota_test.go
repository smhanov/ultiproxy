...
// the operator to start a login through the MCP control plane
// (initiate_oauth_login), not as an error, which MCP get_quota_status renders
...
func TestQuotaNotLoggedIn(t *testing.T) {
	...
	if !strings.Contains(snap.Detail, "not logged in") || !strings.Contains(snap.Detail, "initiate_oauth_login") {
		t.Errorf("Detail = %q, want guidance to use the MCP initiate_oauth_login tool", snap.Detail)
	}
	if strings.Contains(snap.Detail, "ultiproxy login") {
		t.Errorf("Detail = %q, must not point at the removed 'ultiproxy login' subcommand", snap.Detail)
	}
