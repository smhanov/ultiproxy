...
| Tool | What it does |
| :--- | :--- |
| `list_models` | Client-visible model ids with their lane, enabled state, window (`context_limit` / `context_length` / `max_model_len`), output cap (`max_output` / `max_output_tokens`), modalities (`architecture`, `supports_vision`), pricing tag, capabilities and source (`alias` / `discovery` / `default`). Same id set and same metadata precedence as `GET /v1/models`, so the two surfaces cannot drift; ids disabled with `toggle_model` stay listed with `enabled: false`. |
...
| `set_model_alias` | Create/update an alias mapping (provider, upstream, optional `context_limit`, `max_output`, `input_modalities` / `output_modalities`, `pricing_tag`, `input_cost`, `output_cost`); persists in `aliases.json`. Alias metadata overrides live discovery and the static catalog on `GET /v1/models` / `list_models`. |
