...
- **MCP-Native Control Plane**: every knob an operator would touch is an MCP tool at `http://localhost:9050/mcp` -- `add_provider`, `set_model_alias`, `set_provider_timeout`, `get_quota_status`, `initiate_oauth_login`, ... An agent can bootstrap the whole gateway on its own.
